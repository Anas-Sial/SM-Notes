...
// Direct Axios call for external APIs that don't follow our API response structure
export async function axiosGet<T = any>(
  url: string,
  config?: AxiosRequestConfig
): Promise<T> {
  try {
    // Use axios directly instead of axiosInstance to avoid auth interceptors for external APIs
    const response = await axios.get<T>(url, config);
    console.log('Axios GET success:', url, 'Status:', response.status);
    return response.data;
  } catch (error) {
    console.error('Axios GET error:', url, error);
    throw error;
  }
}
